...
const buildBoxes = (count) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    animateds.push(new Animated({
      ...
      animate: delayDurationDelay(4 * ((count / 4 - ((i / 4) | 0)) / (count / 4)), 4, 3 * ((((i / 4) | 0)) / (count / 4)), animate.object({
        angle: t => t * Math.PI * 2,
        delta: t => (i % 4) / 2 + 0.5,
        yDelta: t => Math.sin(Math.min(1, (t <= 0.5 ? t * 2 : -t * 2 + 2) * 4) * Math.PI / 2),
        // rect: animate.object({
      ...
      present: present.styles({
        opacity: present.value(state => 0.5),
